...
export function activate(context: vscode.ExtensionContext) {
	...
	const sessionsProvider = new RegistryProvider(context, rootPath);
	vscode.window.registerTreeDataProvider('veg-registry', sessionsProvider);
...
export class RegistryProvider implements vscode.TreeDataProvider<Image> {

	private _onDidChangeTreeData: vscode.EventEmitter<Image | undefined | void> = new vscode.EventEmitter<Image | undefined | void>();
	readonly onDidChangeTreeData: vscode.Event<Image | undefined | void> = this._onDidChangeTreeData.event;
	...
	getTreeItem(element: Image): vscode.TreeItem {
		return element;
	}

	getChildren(element?: Image): Thenable<Image[]> {
		...
		if (element) {
			...
			for (const s of sessions) {
				...
				const n = new Image(s.sid, l, s.lastUpdate, vscode.TreeItemCollapsibleState.Collapsed)
				nodes.push(n)
			}
			nodes.sort((a: Image,b: Image) => {
				if (sort === "lastUpdate") {
					// newest at the tope
					if(a[sort] > b[sort]) {
						return -1
...
export class Image extends vscode.TreeItem {
	constructor(
