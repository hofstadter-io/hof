...
type FormItem interface {
	...
	// GetLabelText returns the item's label text.
	GetLabelText() string
...
func (f *Form) AddDropDown(label string, options []string, values []any, initialOption int, selected func(option string, optionIndex int)) *Form {
	f.items = append(f.items, NewDropDown().
		SetLabelText(label).
		SetOptions(options, values, selected).
...
func (f *Form) Draw(screen tcell.Screen) {
	...
	for _, item := range f.items {
		labelWidth := TaggedStringWidth(item.GetLabelText())
	...
	buttonsWidth := 0
	for index, button := range f.buttons {
		w := TaggedStringWidth(button.GetLabelText()) + 4
