...
type Box struct {
	...
	eventCapture func(event tcell.Event) tcell.Event
...
// WrapInputHandler wraps an event handler (see InputHandler()) with the
// functionality to capture event (see SetInputCapture()) before passing it
...
func (b *Box) WrapEventHandler(eventHandler func(tcell.Event, func(p Primitive))) func(tcell.Event, func(p Primitive)) {
	return func(event tcell.Event, setFocus func(p Primitive)) {
		if b.eventCapture != nil {
			event = b.eventCapture(event)
		}
		if event != nil && eventHandler != nil {
			eventHandler(event, setFocus)
		}
	}
}
...
func (b *Box) EventHandler() func(event tcell.Event, setFocus func(p Primitive)) {
	return b.WrapEventHandler(nil)
}

// SetEventCapture installs a function which captures key events before they are
// forwarded to the primitive's default key event handler. This function can
// then choose to forward that key event (or a different one) to the default
// handler by returning it. If nil is returned, the default handler will not
// be called.
...
func (b *Box) SetEventCapture(capture func(event tcell.Event) tcell.Event) *Box {
	b.eventCapture = capture
	return b
}

// GetEventCapture returns the function installed with SetEventCapture() or nil
// if no such function has been installed.
func (b *Box) GetEventCapture() func(event tcell.Event) tcell.Event {
	return b.eventCapture
}
...
// SetLabel is a generic function for setting labelerties
func (b *Box) SetLabel(label string, value string) error {
	b.labels[label] = value
	return nil
}

// SetLabels is a generic function for setting labelerties
